...
function [emnx,emny,betax,alphax,betay,alphay] = my_get_slice_twiss(pp, slice_z_input, N_slices)
...
pp_placet = [pp_sort(n_range_1,6)/1e1 pp_sort(n_range_1,1) pp_sort(n_range_1,2) pp_sort(n_range_1,3) pp_sort(n_range_1,4) pp_sort(n_range_1,5)];
...
pp_placet = [pp_sort(n_range,6)/1e1 pp_sort(n_range,1) pp_sort(n_range,2) pp_sort(n_range,3) pp_sort(n_range,4) pp_sort(n_range,5)];
TW = my_calc_twiss(pp_placet);
eps_N = my_calc_emittance(pp_placet); % not yet macro particle
emnx(n) = eps_N(1);
emny(n) = eps_N(2);
...
if(slice_z_fixed)
  if( length(n_slices) < length(slice_z_input) )
    emnx( (length(n_slices)+1):length(slice_z_input)-1) = 0;
    emny( (length(n_slices)+1):length(slice_z_input)-1) = 0;   
    betax( (length(n_slices)+1):length(slice_z_input)-1) = 0;
    alphax( (length(n_slices)+1):length(slice_z_input)-1) = 0;
    betay( (length(n_slices)+1):length(slice_z_input)-1) = 0;   
    alphay( (length(n_slices)+1):length(slice_z_input)-1) = 0;   
  end% if
end% if
